...
def cot_inference(question, dataset_type="gsm8k", max_retries=3):
    ...
    if dataset_type == "gsm8k":
        # 明确要求将数字答案放在最前面
        answer_prompt = "请将最终答案（阿拉伯数字）放在回答的最开头，格式为[答案]。例如：[42]"
    else:  # commonsenseqa
        # 明确要求将字母选项放在最前面
        answer_prompt = "请将最终答案（选项字母）放在回答的最开头，格式为[答案]。例如：[A]"
    ...
    for attempt in range(max_retries):
        try:
            step1_response = client.chat.completions.create(
                model="Qwen/QwQ-32B",
                messages=[
                    {"role": "system", "content": "你是一个严谨的推理助手，请确保你的推理过程清晰且符合逻辑。"},
                    {"role": "user", "content": f"{question} 请逐步推理，但不要给出最终答案。"}
                ],
                temperature=0.7,
    ...
    for attempt in range(max_retries):
        try:
            step2_response = client.chat.completions.create(
                model="Qwen/QwQ-32B",
                messages=[
                    {"role": "system", "content": "你是一个精确的助手，请严格按照要求将答案放在回答开头"},
...
def extract_answer(prediction, dataset_type):
    """从模型输出中提取答案（优先提取开头的答案）"""
    # 尝试提取开头用方括号括起来的答案（新格式）
    bracket_match = re.search(r'^\[([^\]]+)\]', prediction)
    if bracket_match:
        answer_candidate = bracket_match.group(1).strip()
        
        if dataset_type == "gsm8k":
            # 验证是否是数字
            if re.match(r'^\d+$', answer_candidate):
                return answer_candidate
        else:  # commonsenseqa
            # 验证是否是大写字母A-E
            if re.match(r'^[A-E]$', answer_candidate, re.IGNORECASE):
                return answer_candidate.upper()
    
    # 如果没有找到方括号格式的答案，使用原有提取方法
    if dataset_type == "gsm8k":
        # 提取所有数字，取最后一个
        ...
        # 提取第一个大写字母
        letters = re.findall(r'[A-E]', prediction, re.IGNORECASE)
