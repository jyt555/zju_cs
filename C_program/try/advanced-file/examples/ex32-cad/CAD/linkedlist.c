...
void TraverseLinkedListSave(linkedlistADT head, void (*traversefunptr)(void *obj,FILE *,int ), FILE *fp){
	linkedlistADT nodeptr,nptr;
    int i=1;
	if (traversefunptr == NULL) return;
	nodeptr = head->next;
	while (nodeptr != NULL) {
		nptr=nodeptr->dataptr;
		(*traversefunptr)(nodeptr->dataptr,fp,i++); 
