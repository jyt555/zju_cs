...
void DrawLineDSave(void *line, FILE *fp,int i);
...
void TraverseAllListSave();/*保存所有图形对象*/ 
//void DrawLineDSave(void *line, FILE *fp, int i);
...
void DrawLineDSave(void *line, FILE *fp, int i)
{
	...
	//fwrite();
	//sprintf(buf,)
	//fprintf(fp,"%d,%f,%f,%f,%f\n",1,lptr->x1,lptr->y1,lptr->x2,lptr->y2); 
	sprintf(buf,"%d,%f,%f,%f,%f\n",i,lptr->x1,lptr->y1,lptr->x2,lptr->y2); 	
...
void TraverseAllListSave()
{
	FILE *fp;
	fp=fopen("s.dat","w");
    TraverseLinkedListSave(list[LINE], DrawLineDSave,fp);
    /*TraverseLinkedListSave(list[RECT], DrawRect,fp);
    TraverseLinkedListSave(list[ELLIPSE], DrawCenteredEllipse,fp);
    TraverseLinkedListSave(list[STRING], DrawTextT,fp);*/
    fclose(fp);
}
